...
import textModel from "./models/happy.fbx";
  ...
  initThree = () => {
    ...
    function init() {
      ...
      fbxLoader.load(textModel, mesh => {
        ...
        fiveCyclesGroup.position.set(4, 6, -8);
        fiveCyclesGroup.rotation.set(-80, 0, 0);
        fiveCyclesGroup.scale.set(.32, .32, .32);
        fiveCyclesGroup.add(mesh);
      });
      ...
      // 创建五环
      const fiveCycles = [
        { key: 'cycle_0', color: 0xfbb132, position: { x: -250, y: 0, z: 0 }},
      ...
      /*fiveCycles.map(item => {
        //new THREE.TorusGeometry(100, 10, 10, 50)  圆形
        //new THREE.TorusGeometry(100, 10, 10)  六边形
        let cycleMesh = new THREE.Mesh(
            new THREE.TorusGeometry(100, 10, 10, 50), new THREE.MeshLambertMaterial({
      ...
      loader.load('/src/font/STXingkai_Regular.json',function(response){
        var font = response;
